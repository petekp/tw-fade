...
`range` → `reveal` controls how quickly the fade appears and disappears during scroll. `fade-reveal-*` is **global only** — no per-edge reveal utilities.
...
| Old | New |
| --- | --- |
| `--fade-range-*` (e.g. `--fade-range-md`) | `--fade-reveal-*` |
| `--fade-clear-t` | `--fade-clear-top` |
| `--fade-clear-b` | `--fade-clear-bottom` |
| `--fade-clear-l` | `--fade-clear-start` |
| `--fade-clear-r` | `--fade-clear-end` |
